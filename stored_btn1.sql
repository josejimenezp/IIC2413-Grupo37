...
id_instalacion = instalacion.iid;
	...
	END if;
	END LOOP;
RETURN QUERY SELECT foo.fecha, aux.porcentaje_capacidad FROM (SELECT * FROM fechas(fecha_entrada,fecha_salida) EXCEPT (SELECT aux.fecha FROM aux)) as foo WHERE foo.fecha = aux.fecha ORDER BY foo.fecha;
END LOOP;
END;
$$ language plpgsql;