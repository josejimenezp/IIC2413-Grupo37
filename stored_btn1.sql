...
promedio RECORD;
BEGIN
FOR promedio in SELECT AVG(prueba.capacidad_porcentual) as promedio FROM ((SELECT * FROM (SELECT * FROM fecha_table EXCEPT (SELECT * FROM aux)) as foo ORDER BY foo.fechaxd)) as prueba
LOOP
